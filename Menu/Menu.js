(function createChatMenu() {
    ...
    const menuItems = [
        { text: 'Buscar Contenido', action: 'openSearchModal' }, // Acción para abrir el buscador
    ...
    menuItems.forEach(item => {
        ...
        // Si el item tiene un 'href', lo usa como enlace
        if (item.href) {
            a.href = item.href;
        }

        // Si el item tiene una acción, agrega un listener personalizado
        if (item.action === 'openSearchModal') {
            a.href = '#'; // Evita el comportamiento predeterminado
            a.addEventListener('click', (event) => {
                event.preventDefault(); // Evita la navegación
                openSearchModal(); // Llama a la función que abre el modal del buscador
            });
        }
    ...
    // Función para abrir el modal del buscador
